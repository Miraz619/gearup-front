...
type CreateGearFormProps = {
  categories: Category[];
};

export function CreateGearForm({ categories }: CreateGearFormProps) {
  ...
  const onSubmit = async (values: CreateGearFormValues) => {
    ...
    if (!result.success) {
      if (result.fieldErrors) {
        Object.entries(result.fieldErrors).forEach(([field, messages]) => {
          if (field in createGearSchema.shape && messages.length > 0) {
            setError(field as keyof CreateGearFormValues, {
              type: "server",
              message: messages[0],
            });
          }
    ...
    <Card>
      <CardHeader>
        <div className="flex items-start gap-4">
          ...
          <div>
            <CardTitle>Create a new gear listing</CardTitle>

            <CardDescription className="mt-1">
              Add accurate details so customers can easily understand and rent
              your equipment.
            </CardDescription>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div className="grid gap-5 md:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <Input
                id="name"
                placeholder="Example: Canon EOS R50 Camera"
            ...
            <div className="space-y-2 md:col-span-2">
              ...
              <textarea
                id="description"
                rows={6}
                placeholder="Describe the equipment, condition, important features, and suitable uses..."
                disabled={isSubmitting}
